...
|---|---|---|
| POST | `/api/albums` | Create an album (`title`, `cover?`, `color?`, `coverPhoto?`) |
| GET | `/api/albums` | List the logged-in user's albums, each with `count`, `cover`, `color`, and a computed `spineColor` |
| GET | `/api/albums/:id` | Get one album |
| PUT | `/api/albums/:id` | Update `title`, `cover`, `color`, and/or `coverPhoto` |
| DELETE | `/api/albums/:id` | Delete an album **and** all memories/AI insights inside it |
| POST | `/api/albums/:albumId/memories` | Add a memory to an album (`title`, `photo`, `note`, `date`, `mood?`) |
...
- Connected the "Petal & Page" React frontend to this backend end-to-end
- Extended `Album` with `cover` (emoji) and `color` (theme tone) so the frontend's book-shelf UI has real data to render; `spineColor` is computed server-side from `color`
- Extended `Memory` with a required `title` field, matching the frontend's separate title/excerpt display
- Wired auth (login/register/logout/session), album list + create, album detail + memory list, memory create/view/delete, and a live dashboard (real album count + memory count)
- AI Journal, Analytics, Notifications, and Monthly Recap remain on the frontend's mock data — they depend on features scheduled for Week 5 (AI) and Week 6 (Analytics/Notifications)

## What's next (Week 5 per roadmap)

A working AI-powered feature (the AI Memory Companion): reflection generation from a memory's note, automatic mood detection, and monthly AI-generated recaps — via the OpenAI API, surfaced through the `AI Journal` and `Monthly Recap` pages already built on the frontend.
